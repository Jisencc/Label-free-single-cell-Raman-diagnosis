...
OUT_DIR = "./publication_stage1_disease3_reproduction_out_CV5"
...
LOCKED_INPUT_DIR = "./publication_stage1_disease3_inputs_CV5"
LOCKED_RUNS_CSV = os.path.join(
    LOCKED_INPUT_DIR,
    "selected_rep_STAGE1_DISEASE3_LOCKED_RUNS.csv",
)


EXPECTED_SELECTED_REP = 8
...
def train_view_model_locked(
    ...
    Xtr = X[tr_idx]
    ytr = le.transform(df.iloc[tr_idx]["Group"].astype(str).values)
    sc, pca, svm = train_svm_pca(Xtr, ytr, fixed_multi, seed=seed)
    return dict(sc=sc, pca=pca, svm=svm, spec_cols=spec_cols, classes=classes, encoded_classes=encoded_classes, le=le)
...
def load_locked_runs(path: str, expected_rep: int) -> pd.DataFrame:
    ...
    for c in ["final_seed", "all_n_pc", "fp_n_pc", "ch_n_pc"]:
...
def run_locked_disease3():
    ...
    df = df[df["Group"].astype(str).isin(set(classes))].copy()
    if df.empty:
    ...
    out_csv = os.path.join(OUT_DIR, f"stage1_{TASK_NAME}.csv")
    sum_csv = os.path.join(OUT_DIR, f"stage1_{TASK_NAME}_SUMMARY.csv")
    patient_csv = os.path.join(OUT_DIR, f"stage1_{TASK_NAME}_patient_preds.csv")
    splits_csv = os.path.join(OUT_DIR, f"stage1_{TASK_NAME}_splits.csv")
    roc_points_csv = os.path.join(OUT_DIR, f"stage1_{TASK_NAME}_roc_points_ovr.csv")
    ...
    rows = []
    le = LabelEncoder().fit(classes)  # mirrors original script
    encoded_classes = list(le.classes_)
    if encoded_classes != list(ENCODED_CLASS_ORDER):
        raise RuntimeError(f"Encoded class order mismatch: LabelEncoder={encoded_classes}, hardcoded={ENCODED_CLASS_ORDER}")
    ...
    for _, row in locked.iterrows():
        ...
        if any(P_te[v] is None for v in ["all", "fp", "ch"]):
            raise RuntimeError(f"rep={rep}, fold={fold_id}: a view returned empty outer-test probabilities.")
        if not (P_te["all"].index.equals(P_te["fp"].index) and P_te["all"].index.equals(P_te["ch"].index)):
            raise RuntimeError(f"rep={rep}, fold={fold_id}: view patient indices mismatch on outer test.")
        ...
        if SAVE_SPLITS:
            write_row_append(splits_csv, dict(
                ...
                fold=fold_id,
                task=TASK_NAME,
                outer_train_json=json_sorted_list(outer_train),
                outer_test_json=json_sorted_list(outer_test),
            ))
        ...
        if SAVE_PATIENT_PREDS:
            ...
            for i, sk in enumerate(idx_list):
                prow = dict(
                    ...
                    y_true=int(y_te[i]),
                    y_true_name=str(sample_group.loc[sk]),
                    y_pred=int(yhat[i]),
                    y_pred_name=str(encoded_classes[int(yhat[i])]),
                ...
                for k, cname in enumerate(encoded_classes):
                    prow[f"p_{cname}"] = float(P_T[i, k])
                    prow[f"praw_{cname}"] = float(P[i, k])
                    prow[f"all_p_{cname}"] = float(Pte_all[i, k])
        ...
        print(
            f"[{TASK_NAME}] rep {rep} fold {fold_id} "
