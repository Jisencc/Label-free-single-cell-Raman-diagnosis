...
OUT_DIR = "./publication_clinical_reproduction_out_CV3"
...
CLIN_LABEL_COL   = "Diagnosis"   # NOT USED for labels
CLIN_PATIENT_COL = "PatientID"   # unused, kept for reference


LOCKED_INPUT_DIR = "./publication_clinical_inputs_CV3"
...
def preprocess_clinical_locked(df_clin: pd.DataFrame) -> pd.DataFrame:
    ...
    # Parse insight-only numeric cols (does NOT affect model feature selection)
    ...
    # Also normalize subgroup cols if present (analysis-only; does NOT affect model)
...
def run_multimodal_disease3():
    ...
    def _iter_splits():
        if use_stage2_alignment:
            for _, srow in stage2_df.iterrows():
                ...
                # Preserve deterministic order recorded in the split CSV.
    ...
    for rep_num, fold_id, base_seed, outer_seed_used, outer_tries, outer_train_keys, outer_test_keys in _iter_splits():
        ...
        if inner_splits < 2:
            inner_splits = 2  # will fail and fall back if invalid
        ...
        # patient preds (existing file) + accumulate for insights
    ...
    if len(pred_rows_full_for_insight) > 0:
        ...
        # merge clinical info (analysis-only; no effect on training)
