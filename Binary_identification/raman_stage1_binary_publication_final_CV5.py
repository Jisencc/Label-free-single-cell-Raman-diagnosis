...
OUT_DIR = "./publication_stage1_binary_reproduction_out_CV5"
...
LOCKED_INPUT_DIR = "./publication_stage1_binary_inputs_CV5"
LOCKED_RUNS_CSV = os.path.join(
    LOCKED_INPUT_DIR,
    "selected_task_reps_STAGE1_BINARY_LOCKED_RUNS.csv",
)
...
def encoded_class_order_for_task(task: str, classes: List[str]) -> List[str]:
    hard = ENCODED_CLASS_ORDER_BY_TASK.get(str(task))
    if hard is not None:
        if set(hard) != set(classes):
            raise ValueError(f"Hard-coded encoded class order for {task} does not match task classes: {hard} vs {classes}")
        return list(hard)
    # Fallback mirrors LabelEncoder exactly.
    return list(LabelEncoder().fit(classes).classes_)
...
def train_view_model_locked(
    ...
    return dict(sc=sc, pca=pca, svm=svm, spec_cols=spec_cols, classes=classes, encoded_classes=encoded_classes, le=le, best=fixed_multi)
...
def predict_patient_probs_view(df: pd.DataFrame, model: Dict, samples_set: set):
    ...
    P_pat = agg_probs_to_sample_logit(samp, p_cell, model["encoded_classes"])
    return P_pat
...
def load_locked_runs(path: str) -> pd.DataFrame:
    ...
    df = df.copy()
    for c in ["selected_rep", "fold", "final_seed", "all_n_pc", "fp_n_pc", "ch_n_pc"]:
        df[c] = pd.to_numeric(df[c], errors="raise").astype(int)
    for c in ["w_all", "w_fp", "w_ch", "T", "all_C", "fp_C", "ch_C"]:
    ...
    # one selected rep per task, but tasks may differ from each other
...
def run_locked_task(df_all: pd.DataFrame, task: str, disease_label: str, locked_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    classes = [CONTROL_LABEL, disease_label]
    n_classes = 2
    all_cols, wav = get_spec_cols_and_wav(df_all)

    # Subset Raman rows to this task only, avoiding accidental extra labels.
    df = df_all[df_all[RAMAN_LABEL_COL].isin(classes)].copy()
    sample_group = df.groupby(RAMAN_SAMPLE_COL)[RAMAN_LABEL_COL].first()

    out_csv = os.path.join(OUT_DIR, f"stage1_{task}.csv")
    out_patient_csv = os.path.join(OUT_DIR, f"stage1_{task}_patient_preds.csv")
    out_roc_csv = os.path.join(OUT_DIR, f"stage1_{task}_roc_points.csv")
    for f in [out_csv, out_patient_csv, out_roc_csv]:
        if os.path.exists(f):
            os.remove(f)

    le = LabelEncoder().fit(classes)
    encoded_classes = encoded_class_order_for_task(task, classes)
    if list(le.classes_) != encoded_classes:
        raise RuntimeError(f"Encoded class order mismatch for {task}: LabelEncoder={list(le.classes_)}, hardcoded={encoded_classes}")
    rows = []
    patient_rows = []
    roc_rows = []
    ...
    for _, row_locked in locked_rows.sort_values("fold").iterrows():
        ...
        if P_te["all"] is None or P_te["all"].empty:
            raise RuntimeError(f"[{task}] Outer test produced empty probabilities.")

        idx_te = P_te["all"].index
        y_te_str = sample_group.loc[idx_te].values
        y_te = le.transform(y_te_str)

        Pte_all = P_te["all"].loc[idx_te].to_numpy()
        Pte_fp = P_te["fp"].reindex(idx_te).to_numpy()
        Pte_ch = P_te["ch"].reindex(idx_te).to_numpy()
        ...
        P = w_all * Pte_all + w_fp * Pte_fp + w_ch * Pte_ch
        P = P / (P.sum(axis=1, keepdims=True) + 1e-12)
        ...
        out_row = dict(
            ...
            encoded_class_order_json=json.dumps(encoded_classes, ensure_ascii=False),
        ...
        for i, sk in enumerate([str(s) for s in idx_te.tolist()]):
            ...
            patient_rows.append(dict(
                ...
                y_true_name=yname,
                y_pred=ypi,
                y_pred_name=str(encoded_classes[ypi]),
                correct=int(yi == ypi),
                w_all=w_all,
                w_fp=w_fp,
                w_ch=w_ch,
                T=T,
                **{f"p_{encoded_classes[0]}": float(P_T[i, 0]), f"p_{encoded_classes[1]}": float(P_T[i, 1])},
                **{f"praw_{encoded_classes[0]}": float(P[i, 0]), f"praw_{encoded_classes[1]}": float(P[i, 1])},
                **{f"all_p_{encoded_classes[0]}": float(Pte_all[i, 0]), f"all_p_{encoded_classes[1]}": float(Pte_all[i, 1])},
                **{f"fp_p_{encoded_classes[0]}": float(Pte_fp[i, 0]), f"fp_p_{encoded_classes[1]}": float(Pte_fp[i, 1])},
                **{f"ch_p_{encoded_classes[0]}": float(Pte_ch[i, 0]), f"ch_p_{encoded_classes[1]}": float(Pte_ch[i, 1])},
            ))
        ...
        if len(np.unique(y_te)) == 2:
            ...
            roc_rows.append(dict(
                ...
                disease=disease_label,
                positive_class_id=1,
                positive_class_name=str(encoded_classes[1]),
    ...
    sum_csv = os.path.join(OUT_DIR, f"stage1_{task}_SUMMARY.csv")
...
def main():
    ...
    if RAMAN_SAMPLE_COL not in df.columns or RAMAN_LABEL_COL not in df.columns:
        raise ValueError(f"Raman CSV must contain '{RAMAN_SAMPLE_COL}' and '{RAMAN_LABEL_COL}'.")

    all_results = []
    print("Locked task-specific selected reps:")
    for task, sub in locked.groupby("task", sort=True):
        disease = str(sub["disease"].iloc[0])
        rep = int(sub["selected_rep"].iloc[0])
        folds = sorted(sub["fold"].astype(int).tolist())
        print(f"  {task}: disease={disease}, selected_rep={rep}, folds={folds}")
        res = run_locked_task(df, task=task, disease_label=disease, locked_rows=sub)
        all_results.append(res)

    combined = pd.concat(all_results, ignore_index=True) if all_results else pd.DataFrame()
    if len(combined):
        to_csv_safe(combined, os.path.join(OUT_DIR, "stage1_binary_ALL_TASKS.csv"))
        summary = combined.groupby(["task", "disease", "selected_rep"], as_index=False).agg(
            n_rows=("fold", "count"),
            acc_mean=("acc", "mean"),
            acc_std=("acc", "std"),
        ...
        to_csv_safe(summary, os.path.join(OUT_DIR, "stage1_binary_ALL_TASKS_SUMMARY.csv"))
        print("Saved combined summary:", os.path.join(OUT_DIR, "stage1_binary_ALL_TASKS_SUMMARY.csv"))
